...
SIMPLE_JWT = {
    # access_token有效时间
    "ACCESS_TOKEN_LIFETIME": timedelta(days=30) if DEBUG else timedelta(hours=2),
    # refresh_token有效时间
...
CACHES = {
    ...
    'default': {
        ...
        'LOCATION': f'redis://{os.environ.get("REDIS_USER")}:{os.environ.get("REDIS_PASS")}@redis:{os.environ.get("REDIS_PORT")}/0',

        # 缓存的默认超时时间，以秒为单位,默认为 300 秒（5 分钟）
        # 这个过期时间在使用底层缓存API时，作为缺省值提供
        # cache.set("ajanuw", 1)       过期时间为 TIMEOUT
        # cache.set("ajanuw", 1, 10)   10s过期
